...
\begin{frame}{3D RHD simulations (Longo et al. 2025)}
	\begin{columns}
		...
		\vspace{1cm}
		{\scriptsize
		\begin{block}{Code: \texttt{Ratpenat} (Perucho et al. 2010)}
			OpenMPI+OpenMP HRSC 3D RHD code, Marquina 1998 fluxes, PPM recon, Synge EoS,
				TVD-preserving RK
			\\
...
\begin{frame}{SEDs with $\phi=20,60^{\circ}$}
	\begin{columns}
		...
		\begin{column}{.4\textwidth}
			{\scriptsize
			\begin{itemize}
				\item $\eta=0.1\quad\eta_{\rm B}=p_{\rm B}/p_{\rm g}=10^{-2}$
				\item Source at $z=0.003\;(13{\rm Mpc})$ (type CenA)
				\item Fluxes for synchrotron and IC emission
				\item Possible detection of synchrotron by Chandra and IC by CTAO
			\end{itemize}
			}
		\end{column}
	\end{columns}
...
\begin{frame}{Light curve with $\phi=20,60^{\circ}$ and $z=0.007\;(31{\rm Mpc})$}
	\begin{columns}
		\begin{column}{.5\textwidth}
		\centering
		{\scriptsize X-rays $(10^{18}\,{\rm GHz})$}
	...
	\end{columns}
		{\scriptsize
			\begin{itemize}
				...
				\item Chandra is sensitive enough to detect the synchrotron emission for the whole duration of the interaction
			\end{itemize}
		}
\end{frame}
